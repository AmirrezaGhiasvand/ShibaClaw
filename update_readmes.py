...
files = {
    "README.md": {
        ...
        "points": """- ** WebUI Post-Update Login Warning** — Added warning box in the WebUI (Changelog modal and Update panel) to run `shibaclaw reset-admin` in your terminal if login issues occur post-update.
- **⬆️ Version Upgrade** — Updated all configuration and project files to `v0.9.6`."""
    ...
    "README.de.md": {
        ...
        "points": """- ** WebUI Post-Update Login-Warnung** — Warnhinweis in der WebUI (Changelog-Modal und Update-Panel) hinzugefügt, falls nach dem Update Login-Probleme auftreten: Bitte `shibaclaw reset-admin` im Terminal ausführen.
- **⬆️ Versions-Upgrade** — Aktualisierung aller Konfigurations- und Projektdateien auf `v0.9.6`."""
    ...
    "README.es.md": {
        ...
        "points": """- ** Advertencia de inicio de sesión de WebUI post-actualización** — Se agregó un cuadro de advertencia en la WebUI (modal de Changelog y panel de actualización) para ejecutar `shibaclaw reset-admin` en tu terminal si ocurren problemas de inicio de sesión después de actualizar.
- **⬆️ Actualización de versión** — Se actualizaron todos los archivos de configuración y proyecto a la versión `v0.9.6`."""
    ...
    "README.fr.md": {
        ...
        "points": """- ** Avertissement de connexion WebUI post-mise à jour** — Ajout d'une boîte d'avertissement dans la WebUI (modal de Changelog et panneau de mise à jour) pour exécuter `shibaclaw reset-admin` dans votre terminal si des problèmes de connexion surviennent après la mise à jour.
- **⬆️ Mise à niveau de version** — Mise à jour de tous les fichiers de configuration et de projet vers la version `v0.9.6`."""
    ...
    "README.ja.md": {
        ...
        "points": """- ** WebUI アップデート後のログイン警告** — アップデート後にログインの問題が発生した場合にターミナルで `shibaclaw reset-admin` を実行するよう促す警告ボックスを WebUI（変更履歴モーダルおよびアップデートパネル）に追加しました。
- **⬆️ バージョンアップグレード** — すべての設定およびプロジェクトファイルを `v0.9.6` に更新しました。"""
    ...
    "README.pt-BR.md": {
        ...
        "points": """- ** Aviso de login na WebUI pós-atualização** — Adicionada caixa de aviso na WebUI (modal Changelog e painel Update) para executar `shibaclaw reset-admin` no seu terminal caso ocorram problemas de login pós-atualização.
